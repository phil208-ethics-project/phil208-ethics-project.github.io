...
function StudentTablePage() {
  ...
  const [holdStudentsWithLetterLevel, setHoldStudentsWithLetterLevel] =
    useState(false)
  // Retrieve the student data using useLiveQuery
  const students = useLiveQuery(() => db.students.toArray())
  const fictionalGrades = useLiveQuery(() => db.fictional_grades.toArray())
  const informationalGrades = useLiveQuery(() =>
    db.informational_grades.toArray(),
  )
  ...
  const studentsWithGrades = students?.map(student => {
    const studentFictionalGrade = fictionalGrades?.find(
      grade => grade.student_id === student.id,
    )
    const studentInformationalGrade = informationalGrades?.find(
      grade => grade.student_id === student.id,
    )
    const studentSpellingGrade = spellingGrades?.find(
      grade => grade.student_id === student.id,
    )
    const studentReadingLevelGrade = readingLevelGrades?.find(
      grade => grade.student_id === student.id,
    )
  ...
  const filteredStudents = studentsWithGrades?.filter(student => {
    const isMatch = `${student.first_name} ${student.last_name} ${student.id}`
      .toLowerCase()
    ...
    const isFictionalGradeValid =
      fictionalGradeMin <= student.fictional_grade_count &&
      student.fictional_grade_count <= fictionalGradeMax
    const isInformationalGradeValid =
      informationalGradeMin <= student.informational_grade_count &&
      student.informational_grade_count <= informationalGradeMax
    const isReadingGradeLevelValid =
      readingGradeLevelMin <= student.reading_grade_level_count &&
      student.reading_grade_level_count <= readingGradeLevelMax
    ...
    if (holdStudentsWithLetterLevel) {
      ...
      if (
        !isFictionalGradeValid ||
        !isInformationalGradeValid ||
        !isReadingGradeLevelValid
      ) {
        return false
      }
    }
  ...
  return (
    <div className='items-center min-h-screen overflow-x-hidden'>
      <div className='flex-grow overflow-y-auto rounded border-4'>
        ...
        <div className='flex mb-4'>
          <div className='flex-1 mr-2'>
            <label htmlFor='informationalGradeMin'>
              Informational Grade (Min):
            </label>
            <select
          ...
          <div className='flex-1 ml-2'>
            <label htmlFor='informationalGradeMax'>
              Informational Grade (Max):
            </label>
            <select
        ...
        <div className='flex mb-4'>
          <div className='flex-1 mr-2'>
            <label htmlFor='readingGradeLevelMin'>
              Reading Grade Level (Min):
            </label>
            <select
          ...
          <div className='flex-1 ml-2'>
            <label htmlFor='readingGradeLevelMax'>
              Reading Grade Level (Max):
            </label>
            <select
            ...
            {filteredStudents?.map(student => (
              ...
              >
                ...
                <td className='px-6 py-4 text-center'>
                  {student.fictional_grade_count}
                </td>
                <td className='px-6 py-4 text-center'>
                  {student.informational_grade_count}
                </td>
                <td className='px-6 py-4 text-center'>
                  {student.spelling_grade_count}
                </td>
                <td className={`px-6 py-4 text-center`}>
